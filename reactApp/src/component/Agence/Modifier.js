...
function Modifier({ data, fetchData }) {
  ...
  const [nom, setNom] = useState(data.nom_agence);
  ...
  const edit = () => {
    if (nom !== "" && email !== "" && tel !== "" && adresse !== "") {
      ...
      formData.append("nom_agence", nom);
      formData.append("email", email);
      formData.append("tel", tel);
      formData.append("adresse", adresse);
      axios
        .put(`http://127.0.0.1:8000/agence/edit/${data.id}`, formData)
        .then((res) => {
          console.log(res.data);
          fetchData();
          setOpen(false);
          toast.success("Modifier avec succèss!", {
