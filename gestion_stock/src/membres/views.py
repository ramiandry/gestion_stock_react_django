...
from .serializers import AdminSerializer, MembreSerializer, DepartementSerializer
...
def findOneByPk(request,id):
 ...
 #recuperer toute les departements
 
@api_view(['GET'])
def afficherToutDepartement(request):
    departement=Departement.objects.all()
    serializer=DepartementSerializer(departement, many=True)
    return Response(serializer.data)


 #recuperer toute les admins
 
@api_view(['GET'])
def afficherToutAdmin(request):
